              ...
              <div class="panel-body table-slide" style="overflow-x:auto;">
                <table class="table table-striped table-hover">
                  ...
                  <tr>
                    ...
                    <th>{{ $data->author['author'] }}</th>
                    <th><img src="{{ URL::to('/')}}/uploads/{{ $data->image }}" width="100px"></th>
                    <th>{{ $data->category['name'] }}</th>
                    <th>{{ $data->content['content_type_name'] }}</th>
                    <th><a class="{{($data->status == 1)?'btn btn-success':'btn btn-danger'}}" id="status_{{ $data->id }}" onclick="statusUpdate({{ $data->id }}, 'status',{{ $data->status }},'Article')">{{($data->status == 1)?'yes':'no'}}</a></th>
